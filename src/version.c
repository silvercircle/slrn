...
# ifdef HAVE_UNAME
   ...
   if (0 == final_os_name) {
        if (-1 != uname (&u)) {
            size_t len_required = strlen(u.sysname) + strlen(u.release) + strlen(u.machine) + 10;
            final_os_name = (char *)malloc(len_required);
            snprintf(final_os_name, len_required, "%s %s (%s)", u.sysname, u.release, u.machine);
        }
        else {
            final_os_name = (char *)malloc(20);
            snprintf(final_os_name, 19, "%s", "Unix");
        }
